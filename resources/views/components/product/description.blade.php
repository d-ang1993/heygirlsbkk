@if($product_description ?? null)
  ...
  <section class="editorial-spread">
    <div class="spread-left-wrapper">
      <div class="spread-left" @if($left_image) style="background-image: url('{{ $left_image }}');" @endif>
        <div class="spread-left-overlay"></div>
        <div class="spread-left-content">
          <p class="spread-kicker">Why we made this</p>
          <h2 class="spread-title">{{ $product_name }}</h2>
        </div>
      ...
      @if(!empty($left_additional_images))
        <div class="spread-left-images">
          @foreach($left_additional_images as $index => $img)
            <div class="spread-left-image-item" data-bg-image="{{ $img['url'] }}">
              <img src="{{ $img['url'] }}" alt="{{ $img['alt'] }}" loading="lazy" style="display: none;">
            </div>
          @endforeach
        </div>
      @endif
                ...
                <div class="spec-accordion-content">
                  <div class="spec-colors-list">
                    @foreach($colors as $color)
                      <span class="spec-color-item">{{ ucwords(str_replace(['-', '_'], ' ', $color)) }}</span>
                    @endforeach
                  </div>
                </div>
              </div>
                ...
                <div class="spec-accordion-content">
                  <div class="spec-sizes-list">
                    @foreach($sizes as $size)
                      <span class="spec-size-item">{{ strtoupper($size) }}</span>
                    @endforeach
                  </div>
                </div>
              </div>
